...
│   │   ├── message-bubble.tsx  # 单条消息（AI: Markdown / 题卡式问题；用户: 纯文本 + 图片）
│   │   ├── choice-cards.tsx    # 题卡内的单选 / 多选 / 开放输入交互区
...
└── Page
    ...
    └── SplitPanel (左 28%，可拖拽调整 25%-50%)
        ...
        │   ├── Header + 重新开始按钮
        │   ├── MessageList
        │   │   ├── MessageBubble (AI, Markdown 或结构化问题卡)
        │   │   │   └── ChoiceCards (题卡内渲染 single_select / multi_select / open_input)
...
**目标**：左侧聊天区不再只显示纯文本 `A/B/C` 选项，而是将 AI 的问题渲染成完整的结构化题卡。题卡属于对话流的一部分，但视觉上是一张独立内容卡，用户可以直接在卡内完成选择、勾选、补充和提交。

**设计原则**：
- 问题卡出现在左侧 assistant 消息中，属于消息流，而不是页面其他区域的独立表单
- 保留 AI 的自然语言引导文本，但选项区域必须渲染成可交互 UI，而不是纯文字罗列
- 每张题卡只承载一个问题，降低认知负担
- 单选、多选、开放输入三类题型的视觉风格统一，但交互行为明确区分

**支持的题型**：

1. `single_select`
   - 顶部显示题型标签：`单选`
   - 显示 2-4 个可点击选项行，左侧带 A/B/C 编号
   - 点击选项后先高亮选中，500ms 后自动提交（用户可在此期间点击其他选项改变选择）
   - 卡片底部提供一个”或者直接输入你的想法...”的补充输入框，输入后按回车提交
...
2. `multi_select`
   ...
   - 用户可勾选多个选项
   - 底部提供补充输入框，例如”补充其他字段...”
   - 用户点击”确认提交”后，将勾选项和补充内容一起发送

3. `open_input`
   - 顶部显示题型标签：`开放`
   - 不展示预设选项
   - 直接展示大号文本输入框和”发送”按钮
   - 支持 `placeholder` 字段提供输入提示（如”比如:每次要从三个系统里导出数据再手动合并...”）
   - 适用于描述痛点、约束、背景等自由表达场景

**目标视觉效果**：
- 题卡采用浅色背景、细边框、大圆角
- 顶部有题型中文标签和英文类型标识，如 `单选 single_select`
- 问题标题单独占一行，字号略大于正文
- 选项行为整行点击区域，具备 hover / selected 状态
- 输入框和按钮与现有聊天输入区风格保持一致，但属于卡片内部

**题卡状态生命周期**：
- **可交互态**（默认）：用户可点选、勾选、输入，题卡处于活跃状态
- **已提交态**：用户提交后，题卡冻结为只读，高亮显示用户的选择结果，不可再修改。后续对话中用户可看到自己之前的选择

**消息结构**：
- assistant 消息可包含两部分：
  - 自然语言说明文本
  - 结构化问题卡
- 如果该轮主要目的是提问，允许只显示题卡而不额外输出冗长文字
- 用户提交题卡后，仍按普通用户消息进入下一轮对话

**兼容策略**：
- 新协议为主：AI 优先输出结构化问题块
- 老格式兜底：如果 AI 仍输出 `A. xxx / B. xxx / C. xxx`，前端可继续做兼容提取，但最终也应渲染成题卡内的交互区，而不是原样显示文本选项

**协议设计**：

使用 JSON 格式（而非 XML），由 `:::question` / `:::` 标记包裹：

```text
:::question
{“type”:”single_select”,”text”:”这次你想做什么？”,”options”:[“从零做一个新页面/小工具”,”改造现在在用的页面或流程”,”还不确定，先帮我梳理思路”],”allow_custom”:true}
:::
```

```text
:::question
{“type”:”multi_select”,”text”:”这个页面上需要包含哪些字段？(可多选)”,”options”:[“客户名称”,”联系电话”,”所属行业”,”合作状态”],”allow_custom”:true,”custom_placeholder”:”补充其他字段...”}
:::
```

```text
:::question
{“type”:”open_input”,”text”:”能具体描述一下你现在处理这件事最头疼的地方吗？”,”placeholder”:”比如:每次要从三个系统里导出数据再手动合并...”}
:::
```

**为什么用 JSON 而非 XML**：
- LLM 生成 JSON 的格式可靠性远高于 XML（不存在忘闭合标签、属性转义等问题）
- 前端用 `JSON.parse()` 解析比 DOMParser 更健壮
- 用户输入中的 `<` 等字符不会干扰解析
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `type` | `”single_select” \| “multi_select” \| “open_input”` | 是 | 题型 |
| `text` | `string` | 是 | 问题标题 |
| `options` | `string[]` | single/multi 必填 | 选项列表 |
| `allow_custom` | `boolean` | 否 | 是否允许自由补充，默认 `true` |
| `custom_placeholder` | `string` | 否 | 补充输入框的 placeholder |
| `placeholder` | `string` | 否 | open_input 的输入框 placeholder |

前端渲染规则：
- `text` 渲染为题卡问题标题
- `options` 渲染为选项行（single_select 带 A/B/C 编号，multi_select 带复选框）
- `allow_custom` 为 `true` 时，在卡片底部渲染补充输入框
- `type` 决定交互方式：单选 / 多选 / 开放输入

**与现有系统的关系**：
- 不改变 `/api/chat` 的 SSE 主链路
- `:::question` 标记不会被 `StreamParser` 特殊处理 — StreamParser 只识别 `:::prototype` 和 `:::stage:`，`:::question` 作为普通文本透传到客户端
- 结构化问题块的解析在前端消息渲染层（`MessageBubble`）中完成：从消息文本中提取 `:::question\n{...}\n:::` 块，解析 JSON，渲染为题卡组件
- 原型预览能力不受影响
...
### 5.2 核心行为规则
- 一次只问一个问题
- 优先输出结构化问题卡，而不是纯文本 `A/B/C`
- 识别并拆分混合需求
- 根据内容适时生成原型

### 5.3 问题卡输出规范

当 AI 需要用户作答时，应优先输出结构化问题块，由前端渲染为左侧题卡：

- 单选题：使用 `single_select`
- 多选题：使用 `multi_select`
- 开放描述题：使用 `open_input`
...
- 问题文案应简短、口语化、容易选择
- 单选和多选题默认允许用户自由补充
- 不要在输出了结构化问题卡后，再重复输出一遍纯文本 `A/B/C` 选项
